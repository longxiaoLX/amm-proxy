...
pub struct ProxyInitialize<'info> {
    ...
    /// CHECK: Safe. Amm authority, a PDA create with seed = [b"amm authority"]，传进去还有一个 nonce，其实是 bump_seed，amm_authority 是唯一的
...
impl<'info> ProxyInitialize<'info> {
    pub fn handler(
        ...
        amm_anchor::initialize(cpi_ctx, nonce, open_time, init_pc_amount, init_coin_amount)
    }
}
